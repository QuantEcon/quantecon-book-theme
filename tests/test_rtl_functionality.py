...
This test verifies that the script_right_to_left option is properly integrated.
"""
...
def test_rtl_config_option():
    ...
    assert "script_right_to_left = False" in theme_conf_content, \
...
def test_rtl_html_template():
    ...
    assert '{% if theme_script_right_to_left %} dir="rtl"{% endif %}' in layout_content, \
        "HTML layout should conditionally apply dir='rtl' based on theme_script_right_to_left"
...
def test_rtl_python_integration():
    ...
    assert '"theme_script_right_to_left"' in init_content, \
        "Python code should process theme_script_right_to_left setting"
...
def test_rtl_css_styles():
    ...
    assert '[dir=rtl]' in css_content, \
        "Built CSS should contain [dir=rtl] selector"
...
def test_rtl_scss_source():
    ...
    assert '[dir="rtl"]' in rtl_scss_content, \
    ...
    assert '@forward "rtl"' in index_content, \
        "Main SCSS should forward RTL styles"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("  - Configuration option: script_right_to_left = False (default)")
